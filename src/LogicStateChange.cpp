...
std::optional<std::shared_ptr<LogicStateChange>> parseStateChange(const std::string& log_str) {
    ...
    if (std::regex_search(log_str, matches, re)) {
        std::shared_ptr<LogicStateChange> lsc = std::make_shared<LogicStateChange>();
...
std::optional<std::shared_ptr<LogicStateLog>> parseStateLog(const std::string& log_str)
{
    ...
    std::smatch matches;
    if (std::regex_search(log_str, matches, re)) {
        std::shared_ptr<LogicStateLog> lsl = std::make_shared<LogicStateLog>();
...
std::string to_string(const LogicStateChange& logicChange, const LogicStateLog& logicLog){
    ...
    stream << format_time(logicChange.time_stamp) << "," 
           << logicChange.logic.name << "," 
           << logicChange.logic.id << "," 
           << logicChange.state.name << "," 
           << logicLog.message << "," 
           << format_time(logicLog.time_stamp - logicChange.time_stamp) << std::endl;
